...
const LEGACY_SPORT_NAME=/\b(semi[ -]?marathon|marathon|running|course à pied|entraînement|fitness|musculation|cyclisme|vélo|natation|triathlon)\b/i;
...
function migrateLegacyWorkspaceMode(workspace:CustomWorkspace){if(workspace.mode)return workspace;const projectMarkedSport=workspace.state.projects.some(project=>project.mode==="sport");const nameMarkedSport=LEGACY_SPORT_NAME.test([workspace.name,...workspace.state.projects.map(project=>project.name)].join(" "));return projectMarkedSport||nameMarkedSport?withWorkspaceMode(workspace,"sport"):workspace;}
export function loadCustomWorkspaces():CustomWorkspace[]{if(typeof window==="undefined")return[];try{const value:unknown=JSON.parse(window.localStorage.getItem(CUSTOM_WORKSPACES_KEY)||"[]");if(!Array.isArray(value))return[];const parsed=value.filter((item):item is Partial<CustomWorkspace>=>!!item&&typeof item==="object"&&typeof item.id==="string"&&typeof item.name==="string").map(item=>migrateLegacyWorkspaceMode({...item,id:item.id!,name:item.name!,state:normalizeState(item.state)} as CustomWorkspace));const key=`${PURGE_DONE_KEY}:custom`;if(window.localStorage.getItem(key)==="1"){window.localStorage.setItem(CUSTOM_WORKSPACES_KEY,JSON.stringify(parsed));return parsed;}const cleaned=parsed.map(w=>({...w,state:{...w.state,briefing:{...w.state.briefing,tasks:w.state.briefing.tasks.filter(t=>!t.done)}}}));window.localStorage.setItem(CUSTOM_WORKSPACES_KEY,JSON.stringify(cleaned));window.localStorage.setItem(key,"1");return cleaned;}catch{return[];}}
