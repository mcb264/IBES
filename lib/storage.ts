...
export function syncWeeklyCompletedTasks(_log:CompletedTask[],_b:BriefingData){return[] as CompletedTask[];}

function migrateBriefing(b:BriefingData){
  const old=([1,2,3]as const).flatMap(rank=>{const text=b[`p${rank}` as "p1"|"p2"|"p3"].trim();const done=b[`p${rank}Done` as "p1Done"|"p2Done"|"p3Done"];if(!text||done)return[];return[{id:`legacy-priority:${b.date}:${rank}`,text,done:false,effort:b[`p${rank}Effort` as "p1Effort"|"p2Effort"|"p3Effort"]??"normal"}];});
  const existing=(b.tasks??[]).filter(t=>!t.done).map(t=>({...t,done:false}));
  return{...b,p1:"",p2:"",p3:"",p1Done:false,p2Done:false,p3Done:false,tasks:[...old.filter(x=>!existing.some(t=>t.id===x.id)),...existing]};
}
function cleanState(input:Partial<DomainState>|undefined):DomainState{
  const source=input??{};
  const briefing=migrateBriefing({...emptyBriefing(),...(source.briefing??{})});
  return{...defaultDomainState(),...source,briefing:{...briefing,tasks:briefing.tasks.filter(t=>!t.done)},projects:source.projects??[],completedThisWeek:[]};
}
function domainKey(d:Domain){return`ibes:${d}`;}
function parseDomainState(d:Domain):DomainState{try{const raw=window.localStorage.getItem(domainKey(d));if(!raw)return defaultDomainState();return cleanState(JSON.parse(raw));}catch{return defaultDomainState();}}

const MODE_ROUGE_KEY="ibes:mode-rouge",LOAD_SETTINGS_KEY="ibes:load-settings",DAILY_CAPACITY_KEY="ibes:daily-capacity",LOAD_HISTORY_KEY="ibes:load-history",INSIGHT_DISMISS_KEY="ibes:load-insight-dismissed",CUSTOM_WORKSPACES_KEY="ibes:custom-workspaces";
export function loadCustomWorkspaces():CustomWorkspace[]{if(typeof window==="undefined")return[];try{const parsed:CustomWorkspace[]=JSON.parse(window.localStorage.getItem(CUSTOM_WORKSPACES_KEY)||"[]");const cleaned=parsed.map(w=>({...w,state:cleanState(w.state)}));if(JSON.stringify(parsed)!==JSON.stringify(cleaned))window.localStorage.setItem(CUSTOM_WORKSPACES_KEY,JSON.stringify(cleaned));return cleaned;}catch{return[];}}
export function saveCustomWorkspaces(items:CustomWorkspace[]){if(typeof window!=="undefined")window.localStorage.setItem(CUSTOM_WORKSPACES_KEY,JSON.stringify(items.map(w=>({...w,state:cleanState(w.state)}))));}
...
export function saveCustomWorkspace(id:string,state:DomainState){const clean=cleanState(state);saveCustomWorkspaces(loadCustomWorkspaces().map(w=>w.id===id?{...w,state:clean}:w));recordLoadHistory();}

function recordLoadHistory(){if(typeof window==="undefined")return;const settings=loadLoadSettings(),states=[...((["musique","esport","vie"]as Domain[]).map(parseDomainState)),...loadCustomWorkspaces().map(w=>w.state)],date=localDateKey(),capacity=loadDailyCapacity()?.level??null,plannedPoints=states.reduce((n,s)=>n+rawWorkload(s,settings),0);try{const history:LoadHistoryEntry[]=JSON.parse(window.localStorage.getItem(LOAD_HISTORY_KEY)||"[]"),old=history.find(h=>h.date===date),entry:LoadHistoryEntry={date,capacity,plannedPoints,peakPlannedPoints:Math.max(old?.peakPlannedPoints??0,plannedPoints),completedPoints:old?.completedPoints??0,completedPriorities:old?.completedPriorities??0,completedTasks:old?.completedTasks??0,carriedTasks:old?.carriedTasks??0,updatedAt:new Date().toISOString()};window.localStorage.setItem(LOAD_HISTORY_KEY,JSON.stringify([...history.filter(h=>h.date!==date),entry].sort((a,b)=>a.date.localeCompare(b.date)).slice(-90)));}catch{}}
...
export function loadDomainState(d:Domain){if(typeof window==="undefined")return defaultDomainState();try{const state=parseDomainState(d);if(state.briefing.date!==localDateKey())state.briefing={...state.briefing,date:localDateKey(),tasks:state.briefing.tasks.map(t=>({...t,todayDate:undefined,capacityOverrideDate:undefined}))};const clean=cleanState(state);window.localStorage.setItem(domainKey(d),JSON.stringify(clean));return clean;}catch{return defaultDomainState();}}
export function saveDomainState(d:Domain,s:DomainState){if(typeof window!=="undefined"){window.localStorage.setItem(domainKey(d),JSON.stringify(cleanState(s)));recordLoadHistory();}}
