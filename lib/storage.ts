...
function startOfCurrentWeek(){const date=new Date(),day=date.getDay();date.setHours(0,0,0,0);date.setDate(date.getDate()+(day===0?-6:1-day));return localDateKey(date);}
...
function migrateBriefing(b:BriefingData){
  const old=([1,2,3]as const).flatMap(rank=>{const text=b[`p${rank}` as "p1"|"p2"|"p3"].trim();if(!text)return[];return[{id:`legacy-priority:${b.date}:${rank}`,text,done:b[`p${rank}Done` as "p1Done"|"p2Done"|"p3Done"],effort:b[`p${rank}Effort` as "p1Effort"|"p2Effort"|"p3Effort"]??"normal"}];});
  const existing=(b.tasks??[]).map(t=>({...t}));
...
export function loadCustomWorkspaces():CustomWorkspace[]{if(typeof window==="undefined")return[];try{return JSON.parse(window.localStorage.getItem(CUSTOM_WORKSPACES_KEY)||"[]");}catch{return[];}}
