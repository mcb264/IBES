...
export function syncWeeklyCompletedTasks(log:CompletedTask[],briefing:BriefingData):CompletedTask[]{
 ...
 const kept=current.filter(e=>{
   if(e.kind==="priority"&&priorityIds.has(e.id)){
     const rank=e.priorityRank??Number(e.id.split(":").pop()) as 1|2|3;
     return Boolean(briefing[`p${rank}Done` as "p1Done"|"p2Done"|"p3Done"]);
   }
 ...
 const priorityAdditions=([1,2,3] as const).flatMap(rank=>{
   const text=briefing[`p${rank}` as "p1"|"p2"|"p3"].trim();
   const done=briefing[`p${rank}Done` as "p1Done"|"p2Done"|"p3Done"];
   const id=`priority:${briefing.date}:${rank}`;
   return done&&text&&!existing.has(id)?[{id,text,date:briefing.date,kind:"priority" as const,priorityRank:rank}]:[];
 });
