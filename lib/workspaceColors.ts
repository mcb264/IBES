...
export type ColoredWorkspace=CustomWorkspace&{color?:string};
...
export function workspaceColor(workspace:CustomWorkspace){
  const color=(workspace as ColoredWorkspace).color;
  return color&&/^#[0-9a-f]{6}$/i.test(color)?color:fallbackWorkspaceColor(workspace.id);
}

export function withWorkspaceColor(workspace:CustomWorkspace,color:string):ColoredWorkspace{
