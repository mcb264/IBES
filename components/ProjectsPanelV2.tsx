...
function currentSportStep(task: TaskItem) {
  ...
  const index = Math.min(
    task.recurrenceHistory?.length ?? 0,
    sportTask.sportSteps.length - 1,
  );
  return {
    label: sportTask.sportSteps[index],
    index,
    total: sportTask.sportSteps.length,
  };
}

export default function ProjectsPanelV2({
  projects,
  tasks,
  accentColor,
  onChange,
  ...
  const normalizeOrder = (items: Project[]) =>
  ...
  const toggleAction = (id: string) => {
    ...
    if (task.recurringTarget) {
      onTasksChange?.(
        tasks.map((item) =>
          item.id === id ? completeRecurringTask(item) : item,
        ),
      );
      return;
    }
  ...
  const toggleWaiting = (id: string) => {
    ...
    patchAction(id, {
      ...
      waitingSince: !task.waiting ? localDateKey() : undefined,
  ...
  const startSportProgram = (task: TaskItem) => {
    ...
    setSportProgramText((sportTask.sportSteps ?? []).join("\n"));
    setMenu(null);
  };

  const saveSportProgram = (id: string) => {
    const steps = sportProgramText
      .split("\n")
      .map((line) => line.trim())
      .filter(Boolean);
    onTasksChange?.(
  ...
  const reorder = (projectId: string, targetId: string) => {
    ...
    onTasksChange(
      tasks.map((task) =>
        task.projectId === projectId ? reordered[index++] : task,
      ),
    );
    setDragged(null);
  };

  const editingSportTask = editingSportProgram
    ? tasks.find((task) => task.id === editingSportProgram)
    : null;
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-end justify-between gap-4">
        <div>
          <p className="text-[10px] font-mono uppercase tracking-[.22em] text-muted">
            Pilotage
          </p>
          <h2 className="font-display text-3xl mt-1" style={{ color: accentColor }}>
            Projets
          </h2>
          <p className="text-sm text-muted mt-2">
            La couleur identifie le grand projet sans envahir l’interface.
          </p>
        </div>
        <button
      ...
      {creating && (
        <div
          className="rounded-2xl border border-white/10 bg-panel p-5 space-y-5"
        ...
        >
          <div>
            ...
            <p className="mt-1 text-xs text-muted">
              Choisis son fonctionnement. Un projet Sport reste un projet IBES normal.
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <input
              ...
              className="bg-graphite border border-white/10 rounded-lg px-4 py-3"
            />
            <input
              value={goal}
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {(["standard", "sport"] as ProjectMode[]).map((mode) => {
              ...
              const sport = mode === "sport";
              return (
                <button
                  key={mode}
                  type="button"
                  onClick={() => setProjectMode(mode)}
                  className="rounded-xl border bg-graphite/50 p-4 text-left transition"
                  style={{
                    borderColor: active ? accentColor : "rgba(255,255,255,.10)",
                  }}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-display text-base">
                      {sport ? "Mode Sport" : "Projet standard"}
                    </span>
                    {active && <span style={{ color: accentColor }}>✓</span>}
                  </div>
                  <p className="mt-1 text-xs text-muted">
                    {sport
                      ? "Séances évolutives et outils sportifs à venir."
                      : "Actions, échéances, charges et récurrences classiques."}
                  </p>
                </button>
              );
            })}
          ...
          {projectMode === "sport" && (
            <div className="rounded-xl border border-white/10 bg-white/[.02] px-4 py-3">
              <p className="text-xs" style={{ color: accentColor }}>
                Sport activé
              </p>
              <p className="mt-1 text-xs text-muted">
                Exemple : Course · 3×/semaine avec 25 min → 25 min → 35 min → 30 min… sans créer une tâche par séance.
              </p>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="flex-1 bg-graphite border border-white/10 rounded-lg px-4 py-3 text-sm"
            />
            <button
              onClick={addProject}
              disabled={!name.trim()}
              className="rounded-lg px-5 py-3 font-mono text-xs uppercase text-graphite disabled:opacity-40"
        ...
        {ordered.map((project, projectIndex) => {
          ...
          return (
            ...
            >
                ...
                <div className="flex items-start gap-4">
                  <button
                    ...
                    onClick={() =>
                      onChange(
                        projects.map((item) =>
                          item.id === project.id ? { ...item, done: !item.done } : item,
                        ),
                      )
                    }
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="cursor-grab select-none text-muted/40" title="Glisser pour réordonner">
                        ⠿
                      </span>
                      <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: accentColor }} />
                      <h3
                        className={`font-display text-2xl ${project.done ? "line-through" : ""}`}
                        style={{ color: accentColor }}
                      >
                        {project.name}
                      </h3>
                      {sportProject.mode === "sport" && (
                        <span
                          className="rounded-full border border-white/10 px-2 py-1 text-[8px] font-mono uppercase"
                          style={{ color: accentColor }}
                        >
                          Sport
                        </span>
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">
                      <span className="rounded-full border border-white/10 bg-graphite px-2.5 py-1 text-[10px] font-mono uppercase text-muted">
                        {open.length} à faire
                      </span>
                      {done.length > 0 && (
                        <span className="rounded-full border border-white/10 bg-graphite px-2.5 py-1 text-[10px] font-mono uppercase text-muted">
                      ...
                      {editingDate === project.id ? (
                        ...
                        <button
                          data-no-project-drag
                          onClick={() => setEditingDate(project.id)}
                          className="rounded-full border border-white/10 bg-graphite px-2.5 py-1 text-[10px] font-mono uppercase text-muted"
                        >
                          {project.dueDate
                  ...
                  <div data-no-project-drag className="relative">
                    <button
                      onClick={() => setMenu(menu === `p:${project.id}` ? null : `p:${project.id}`)}
                      className="px-2 py-1 text-muted"
                    >
                      •••
                    </button>
                    {menu === `p:${project.id}` && (
                      <div className="absolute right-0 top-8 z-20 w-44 rounded-lg border border-white/10 bg-graphite p-1 shadow-xl">
                        <button
                          disabled={projectIndex === 0}
                          onClick={() => moveProject(project.id, -1)}
                          className="w-full px-3 py-2 text-left text-xs text-muted disabled:opacity-30"
                        >
                          ↑ Monter
                        </button>
                        <button
                          disabled={projectIndex === ordered.length - 1}
                          onClick={() => moveProject(project.id, 1)}
                          className="w-full px-3 py-2 text-left text-xs text-muted disabled:opacity-30"
                        >
                          ↓ Descendre
                        ...
                        <button
                          ...
                          className="w-full px-3 py-2 text-left text-xs text-alert"
                        >
                          Supprimer le projet
                        </button>
                      </div>
              ...
              <div
                data-no-project-drag
                className="border-t border-white/10 px-5 sm:px-6 py-5 rounded-b-2xl bg-white/[.015]"
              >
                <div className="mb-3 flex items-center justify-between">
                  <p
                    className="text-[10px] font-mono uppercase tracking-[.18em]"
                    style={{ color: accentColor }}
                  >
                    Actions
                  </p>
                  {sportProject.mode === "sport" && (
                    <span className="text-[9px] font-mono uppercase text-muted">
                      Séances évolutives disponibles
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  {open.length === 0 && (
                    <p className="text-sm text-muted py-2">Aucune action ouverte.</p>
                  )}
                  ...
                  {open.map((task, taskIndex) => {
                    ...
                    const sportStep =
                      sportProject.mode === "sport" ? currentSportStep(task) : null;
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          opacity: task.waiting ? 0.68 : 1,
                        }}
                      >
                        <button
                          onClick={() => toggleAction(task.id)}
                          disabled={recurrenceComplete}
                          className="w-5 h-5 rounded border border-white/20 text-transparent disabled:opacity-30"
                        >
                          ✓
                        </button>
                        ...
                        <div className="min-w-0 flex-1">
                          {editingActionText === task.id ? (
                            <input
                              ...
                              onBlur={() => renameAction(task.id)}
                              onKeyDown={(event) => {
                                if (event.key === "Enter") renameAction(task.id);
                                if (event.key === "Escape") {
                                  setEditingActionText(null);
                                  setActionText("");
                                }
                          ...
                          {sportStep && (
                            <p className="mt-1 text-[11px] text-muted">
                              <span style={{ color: accentColor }}>Prochaine séance · </span>
                              {sportStep.label}
                            </p>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          {!!task.recurringTarget && (
                            <span
                              className="rounded-full border border-white/10 px-2 py-1 text-[8px] font-mono uppercase"
                              style={recurrenceComplete ? { color: accentColor } : undefined}
                            >
                              {progress.count}/{progress.target} semaine
                            </span>
                          )}
                          {sportStep && (
                            <span className="rounded-full border border-white/10 px-2 py-1 text-[8px] font-mono uppercase text-muted">
                              {sportStep.index + 1}/{sportStep.total}
                            </span>
                          )}
                          <span className="rounded-full border border-white/10 px-2 py-1 text-[8px] font-mono uppercase text-muted">
                            {EFFORT_LABEL[task.effort ?? "normal"]}
                          </span>
                          ...
                          {editingActionDate === task.id ? (
                            <input
                              ...
                              onBlur={(event) => {
                                patchAction(task.id, { dueDate: event.target.value || undefined });
                                setEditingActionDate(null);
                              }}
                              className="bg-graphite rounded-md px-2 py-1 text-[9px] border border-white/10"
                            />
                          ) : (
                            task.dueDate && (
                              <button
                                onClick={() => setEditingActionDate(task.id)}
                          ...
                          <div className="relative">
                            <button
                              onClick={() => setMenu(menu === `a:${task.id}` ? null : `a:${task.id}`)}
                            ...
                            {menu === `a:${task.id}` && (
                              <div className="absolute right-0 top-7 z-30 w-56 rounded-lg border border-white/10 bg-graphite p-1 shadow-xl">
                                <button
                                  onClick={() => startRenamingAction(task)}
                                  className="w-full px-3 py-2 text-left text-xs text-muted"
                                >
                                  Renommer
                                </button>
                                <p className="px-3 pt-2 pb-1 text-[9px] font-mono uppercase text-muted">
                                  Charge
                                </p>
                                {(
                                  [
                                  ...
                                  <button
                                    key={value}
                                    onClick={() => setEffort(task.id, value)}
                                    className="w-full px-3 py-1.5 text-left text-xs"
                                    style={
                                      task.effort === value || (!task.effort && value === "normal")
                                        ? { color: accentColor }
                                        : undefined
                                    }
                                  >
                                ...
                                {[1, 2, 3, 4, 5, 6, 7].map((times) => (
                                  <button
                                    key={times}
                                    onClick={() => setRecurrence(task.id, times)}
                                    className="w-full px-3 py-1.5 text-left text-xs"
                                    style={task.recurringTarget === times ? { color: accentColor } : undefined}
                                  >
                                    {times}× / semaine
                                  </button>
                                ))}
                                {!!task.recurringTarget && (
                                  <button
                                    onClick={() => setRecurrence(task.id)}
                                    className="w-full px-3 py-1.5 text-left text-xs text-muted"
                                  >
                                    Aucune récurrence
                                  </button>
                                )}
                                {sportProject.mode === "sport" && (
                                  <>
                                    <div className="my-1 border-t border-white/10" />
                                    <p
                                      className="px-3 pt-2 pb-1 text-[9px] font-mono uppercase"
                                      style={{ color: accentColor }}
                                    >
                                      Mode Sport
                                    </p>
                                    <button
                                      onClick={() => startSportProgram(task)}
                                      className="w-full px-3 py-2 text-left text-xs text-muted"
                                    >
                                      Séances évolutives
                                    </button>
                ...
                {editingSportTask?.projectId === project.id && (
                  <div className="mt-3 rounded-xl border border-white/10 bg-graphite/60 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-xs font-medium" style={{ color: accentColor }}>
                          Séances évolutives · {editingSportTask.text}
                        </p>
                        <p className="mt-1 text-[11px] text-muted">
                          1 ligne = 1 séance. Après chaque validation, IBES affiche automatiquement la suivante.
                        </p>
                      </div>
                      <button
                        onClick={() => {
                    ...
                    <textarea
                      ...
                      onChange={(event) => setSportProgramText(event.target.value)}
                      placeholder={
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3">
                      <p className="text-[10px] text-muted">
                        Tu peux coller tout ton programme d’un coup.
                      </p>
                      <div className="flex gap-2">
                        <button
                          ...
                          className="rounded-lg border border-white/10 px-3 py-2 text-[10px] font-mono uppercase text-muted"
                        >
                          Annuler
                        </button>
                        <button
                          onClick={() => saveSportProgram(editingSportTask.id)}
                          className="rounded-lg px-3 py-2 text-[10px] font-mono uppercase text-graphite"
                          style={{ backgroundColor: accentColor }}
                        >
                          Enregistrer
                        </button>
                ...
                {done.length > 0 && (
                  <details className="mt-3">
                    <summary className="cursor-pointer text-[10px] font-mono uppercase text-muted">
                      {done.length} terminée{done.length > 1 ? "s" : ""}
                    </summary>
                    <div className="mt-2 space-y-1">
                      {done.map((task) => (
                        <div
                          key={task.id}
                          className="flex items-center gap-2 rounded-lg border border-white/10 px-4 py-2 text-sm text-muted"
                        >
                          <button
                            onClick={() => toggleAction(task.id)}
                          ...
                          >
                            ...
                            {editingActionText === task.id ? (
                              <input
                                ...
                                className="min-w-0 flex-1 rounded border border-white/20 bg-graphite px-2 py-1 text-sm text-ink outline-none"
                                aria-label="Renommer l’action"
                              />
                            ) : (
                              <span className="line-through flex-1 truncate">{task.text}</span>
                            )}
                            <span className="text-[9px] font-mono uppercase">Réouvrir</span>
                          </button>
                          <button
                            onClick={() => startRenamingAction(task)}
                            title="Renommer l’action"
                            className="text-muted/50 hover:text-ink"
                          >
                            ✎
                          </button>
                          <button
                            onClick={() => deleteAction(task.id)}
                            className="text-muted/30 hover:text-alert"
                          >
                            ×
                          </button>
                        </div>
                ...
                {sportProject.mode === "sport" && open.length === 0 && (
                  <p className="mt-3 text-[11px] text-muted">
                    Astuce : crée « Course », règle-la à 3×/semaine puis ajoute ses séances évolutives.
                  </p>
                )}
                ...
                {onTasksChange && (
                  <div className="mt-4 flex gap-2">
                    <input
                      ...
                      onKeyDown={(event) => {
                        if (event.key === "Enter") addAction(project.id);
                      }}
                      placeholder={
                        sportProject.mode === "sport"
                          ? "Ajouter une séance / action…"
                          : "Ajouter une action…"
                      }
                      className="min-w-0 flex-1 bg-transparent border-b border-white/10 px-1 py-2 text-sm outline-none"
                    />
                    <button
                      onClick={() => addAction(project.id)}
                      disabled={!(newActions[project.id] || "").trim()}
                      className="text-[10px] font-mono uppercase tracking-widest disabled:opacity-30"
                      style={{ color: accentColor }}
                    >
                      Ajouter
                    </button>
