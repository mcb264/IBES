...
export default function CustomProjectConsole({id}:{id:string}){
  ...
  return <main className="min-h-screen flex flex-col">
    <header className="border-b px-6 py-4 flex items-center gap-3" style={{borderColor:`${color}33`}}><Link href="/" className="font-mono text-[11px] uppercase tracking-widest text-muted border rounded-full px-3 py-1.5" style={{borderColor:`${color}44`}}>← Accueil</Link><div><h1 className="font-display text-xl" style={{color}}>{workspace.name}</h1><p className="text-[10px] font-mono text-muted">PROJET</p></div></header>
    ...
    <section className="flex-1 px-6 py-8 max-w-3xl w-full mx-auto">
      {tab==="projects"&&<ProjectsPanel accentColor={color} projects={workspace.state.projects} tasks={workspace.state.briefing.tasks} onChange={projects=>setState(s=>({...s,projects}))} onTasksChange={tasks=>setState(s=>({...s,briefing:{...s.briefing,tasks}}))}/>} 
