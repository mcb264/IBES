    ...
    <main className="min-h-screen flex items-center justify-center px-6">
      <form onSubmit={submit} className="w-full max-w-sm space-y-4">
        <div>
          ...
          <h1 className="font-display text-3xl font-bold mt-2">Créer un accès</h1>
          <p className="mt-2 text-sm text-muted">Choisis ton identifiant et ton mot de passe. Les comptes existants peuvent aussi être initialisés ici.</p>
        </div>
        ...
        <input className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3" placeholder="Identifiant" autoComplete="username" value={login} onChange={(event) => setLogin(event.target.value)} required autoFocus={mode === "new"} />
        ...
        <button className="w-full rounded-xl bg-ink text-graphite px-4 py-3 font-semibold disabled:opacity-50" disabled={loading}>{loading ? "Initialisation…" : "Créer mon accès"}</button>
